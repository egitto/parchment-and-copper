...
if (!read_text_file(variable_storage)){
	...
	x = read_text_file(variable_storage);
	x = JSON.parse(x)
}
...
client.on('message', msg => {
  ...
  console.log(obey_this+' '+msg.content);
...
function parse_command_phrase(phrase){
	  ...
	  var x = /^change log channel (.+)$/
		if (phrase.match(x)) {
			log_channel_name = phrase.replace(x,'$1')
